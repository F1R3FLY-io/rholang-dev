...
layout: page
title: Receiving
description: Learn how to listen on channels, trigger comm events, and build simple contracts in Rholang
---
...
![Keys in the freezer reminder note](https://raw.githubusercontent.com/rholangcoop/rholang.org/source/content/tutorials/images/receiving-Keys.png)
...
When some message shows up on `channel` it gets received and is referred to as `message`. The code inside the curly braces, `{}`, is called a "continuation" and it runs when a message is received.

Lines that start with `//` are called comments. They're just there for human coders and don't affect the way the program runs at all. They're a good idea, and you should use them; anyone who reads your code will appreciate them (including your future self).

## Comm Events

![Pizza shop can receive messages on its channel](https://raw.githubusercontent.com/rholangcoop/rholang.org/source/content/tutorials/images/receiving-pizza.png)
...
### Exercise
Send that message to a different channel like `coffeeShop`. Did the acknowledgement print? Is anything left in the tuplespace?

![Coffee shop channel](https://raw.githubusercontent.com/rholangcoop/rholang.org/source/content/tutorials/images/receiving-coffee.png)
...
![Looking for messages before they arrive](https://raw.githubusercontent.com/rholangcoop/rholang.org/source/content/tutorials/images/receiving-lookingForMessages.png)
...
![Writing a contract to keep up with orders](https://raw.githubusercontent.com/rholangcoop/rholang.org/source/content/tutorials/images/receiving-pizzaBurning.png)
...
This is different from a normal `for` because it has a double arrow `<=` rather than a single arrow `<-`. The only difference between the persistent for and a contract comes when we start talking about blockchains. For now you can think of them as the same thing.
